...
from roop.utilities import conditional_download, resolve_relative_path, is_image, is_video, get_destfilename_from_path, \
    from_path_to_array_index, create_virtual_paths_list
...
def process_frames(is_batch: bool, source_face: Face, target_face: Face, temp_frame_paths: List[str],
                   update: Callable[[], None]) -> None:
    for temp_frame_path in temp_frame_paths:
        temp_frame = roop.globals.temp_frames_buffer[from_path_to_array_index(temp_frame_path)]
        ...
        if result is not None:
            if is_batch:
                ...
                roop.globals.temp_frames_buffer[from_path_to_array_index(temp_frame_path)] = result
        if update:
            update()
...
def process_video(source_face: Any, target_face: Any, temp_frame_paths: List[str]) -> None:
    paths = create_virtual_paths_list()
    roop.processors.frame.core.process_video(source_face, target_face, paths, process_frames)
